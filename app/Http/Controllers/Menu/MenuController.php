...
use App\Http\Requests\Menu\Menu\BlockMenuRequest;
use App\Http\Requests\Menu\Menu\DeleteMenuRequest;
use App\Http\Requests\Menu\Menu\EditMenuRequest;
use App\Http\Requests\Menu\Menu\NewMenuRequest;
use App\Http\Requests\Menu\Menu\RecoverMenuRequest;
...
class MenuController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return datatables($collection)
                ...
                ->addColumn('type', function ($row) {
                    return '<span class="fe-mouse-default" data-toggle="tooltip" data-placement="top" data-value="' . $row->getMenuOption['name'] . '" title="' . $row->getMenuOption['description'] . '">' . $row->getMenuOption['name'] . '</span>';
                })
                // coluna visível
                ...
                ->addColumn('action', function ($row) {
                    // visualizar
                    if (app('router')->has('menu.view') && MenuItem::getMenuItemDeleted('menu.view')['list']) {
                        if (Permission::buttonPermission('btn-modal-view-menu') && !MenuItem::getMenuItemBlocked('menu.view')['list']) {
                    ...
                    // editar
                    if (app('router')->has('menu.edit') && MenuItem::getMenuItemDeleted('menu.edit')['list']) {
                        if (Permission::buttonPermission('btn-modal-edit-menu') && !MenuItem::getMenuItemBlocked('menu.edit')['list']) {
                    ...
                    // excluir
                    if (app('router')->has('menu.delete') && MenuItem::getMenuItemDeleted('menu.delete')['list']) {
                        if (Permission::buttonPermission('btn-modal-delete-menu') && !MenuItem::getMenuItemBlocked('menu.delete')['list'] && $row->id != 9) {
    ...
    public function listDeleted(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return datatables($collection)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    // recuperar
                    if (app('router')->has('menu.recover') && MenuItem::getMenuItemDeleted('menu.recover')['list']) {
                        if (Permission::buttonPermission('btn-modal-recover-menu') && !MenuItem::getMenuItemBlocked('menu.recover')['list']) {
        ...
        return view('menu.menu.list-deleted');
    }
