...
use App\Http\Requests\Management\Admin\EditAdminRequest;
use App\Http\Requests\Management\Admin\NewAdminRequest;
use App\Http\Requests\Management\Admin\RecoverAdminRequest;
...
class AdminController extends Controller
{
    ...
    public function list(Request $request)
    {
            ...
            return datatables($collection)
                ...
                ->addColumn('email', function ($row) {
                    if ($row->email_verified_at) {
                        if (app('router')->has('admin.send.email') && Permission::routePermission('admin.send.email') && !MenuItem::getMenuItemBlocked('admin.send.email')['button'] && MenuItem::getMenuItemDeleted('admin.send.email')['button'] && $row->id != auth()->id()) {
        ...
        $dash = PageHelpers::page('6');
        $page = PageHelpers::page('7');
        $list = PageHelpers::page('8');
        $add  = PageHelpers::page('10');

        return view('management.admins.tables.all.page', compact('dash', 'page', 'list', 'add'));
    }
    ...
    public function listDeleted(Request $request)
    {
        ...
        $dash = PageHelpers::page('6');
        $page = PageHelpers::page('8');
        $list = PageHelpers::page('7');
        $add  = PageHelpers::page('10');

        return view('management.admins.tables.deleted.page', compact('dash', 'page', 'list', 'add'));
    }
