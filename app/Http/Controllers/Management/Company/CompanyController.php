...
use App\Http\Requests\Management\Company\NewAdminCompanyRequest;
use App\Http\Requests\Management\Company\NewCompanyRequest;
...
class CompanyController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return datatables($collection)
                ...
                ->addColumn('name', function ($row) {
                    if (app('router')->has('company.admin.store') && Permission::routePermission('company.admin.store') && !MenuItem::getMenuItemBlocked('company.admin.store')['button'] && MenuItem::getMenuItemDeleted('company.admin.store')['button'] && auth()->user()['admin'] == 1) {
                        $name = '<span data-id="' . $row->id . '" data-logo="' . $row->logo . '" data-name="' . $row->name . '" class="status fe-pointer btn-modal-new-admin-company" data-toggle="tooltip" data-placement="top" title="clique aqui para criar um novo administrador nesta empresa">' . $row->name . '</span>';
                    } else {
                        $name = $row->name;
                    }
                    return $name;
                ...
                ->addColumn('email', function ($row) {
                    if (app('router')->has('company.send.email') && Permission::routePermission('company.send.email') && !MenuItem::getMenuItemBlocked('company.send.email')['button'] && MenuItem::getMenuItemDeleted('company.send.email')['button'] && $row->email != auth()->user()->email) {
                ...
                ->addColumn('action', function ($row) {
                    // visualizar
                    if (app('router')->has('company.view') && MenuItem::getMenuItemDeleted('company.view')['button']) {
                        if (Permission::routePermission('company.view') && !MenuItem::getMenuItemBlocked('company.view')['button']) {
     ...
     * Armazenar dados recém-criado no armazenamento.
     *
     * @param NewAdminCompanyRequest $request
     * @return JsonResponse
     */
    public function storeAdmin(NewAdminCompanyRequest $request)
    {
        $collection = User::create([
            'name'           => $request->name_new_admin_company,
            'email'          => $request->email_new_admin_company,
    ...
    public function listAdmins(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return datatables($collection)
                ...
                ->addColumn('action', function ($row) {
                    // visualizar
                    if (app('router')->has('admin.view') && MenuItem::getMenuItemDeleted('admin.view')['button']) {
                        if (Permission::routePermission('admin.view') && !MenuItem::getMenuItemBlocked('admin.view')['button']) {
                    ...
                    if ($row->id != auth()->id() && CompanyAccess::getCompanyAccessUser($row->id)['company_id'] == CompanyAccess::getCompanyAccessUser(auth()->id())['company_id'] || $row->id != auth()->id() && CompanyAccess::getCompanyAccessUser(auth()->id())['company_id'] == 1) {
                        ...
                        // excluir
                        if (app('router')->has('admin.delete') && MenuItem::getMenuItemDeleted('admin.delete')['button']) {
                            if (Permission::routePermission('admin.delete') && !MenuItem::getMenuItemBlocked('admin.delete')['button']) {
        ...
        if (!Company::id() || Company::id() != $request->get('id') && Company::id() != 1) {
            // notificar
