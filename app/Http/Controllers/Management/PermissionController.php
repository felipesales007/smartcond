...
class PermissionController extends Controller
{
     ...
     * @throws Exception
     */
    public function listWithout(Request $request)
    {
        ...
        $collection = User::query()
            ...
            ->when(Company::id() > 1, function ($query) {
                $query
                    ->leftJoin('company_accesses', 'company_accesses.user_id', '=', 'users.id')
                    ->where('company_accesses.company_id', Company::id());
            })
     ...
     * @throws Exception
     */
    public function listWith(Request $request)
    {
        // filtragem do datatable
        $search = !empty($_GET['search']) ? $_GET['search'] : '';
                    ...
                    if ($row->id != auth()->id() && CompanyAccesses::getCompanyAccessUser($row->id)['company_id'] == CompanyAccesses::getCompanyAccessUser(auth()->id())['company_id'] || $row->id != auth()->id() && EntityAccess::getEntityAccessUser($row->id)['entity_id'] || $row->id != auth()->id() && CompanyAccesses::getCompanyAccessUser(auth()->id())['company_id'] == 1) {
                        if (app('router')->has('permission.user.edit') && MenuItem::getMenuItemDeleted('permission.user.edit')['button']) {
                            if (Permission::routePermission('permission.user.edit') && !MenuItem::getMenuItemBlocked('permission.user.edit')['button']) {
                                ...
                                $btn = '<a href="javascript:void(0)" class="btn btn-sm btn-icon btn-outline-success opacity-2 disabled"><i class="fas fa-lock-open mr-2"></i>editar acesso</a>';
                            }
                        } else {
                            $btn = null;
                        }
                    } else {
                        $btn = '<a href="javascript:void(0)" class="btn btn-sm btn-icon btn-outline-success opacity-2 disabled"><i class="fas fa-lock mr-2"></i>editar acesso</a>';
                    }
    ...
    public function edit(Request $id)
    {
        ...
        $groups = Group::select('groups.id as group', 'groups.name', 'icon', 'user_level_id as level_id', 'user_levels.name as level')
        ...
        $permissions = Permission::select('groups.id as group', 'routes.id', 'routes.view', 'routes.url', 'menu_items.name as name',
            'menu_items.button as button', 'menu_items.button as button', 'menu_items.main as main', 'routes.description')
