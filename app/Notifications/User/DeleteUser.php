...
class DeleteUser extends Notification
{
    ...
    public function toMail()
    {
        ...
        $mailMessage->subject('Notificação de exclusão de usuário');
        $mailMessage->greeting('Olá ' . FormatHelpers::first_word($this->name) . ',');
        $mailMessage->line('Você está recebendo este e-mail porque foi realizado a <span class="text-warning">exclusão do seu usuário</span> no sistema ' . config('app.name'));
