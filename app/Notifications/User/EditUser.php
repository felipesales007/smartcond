...
class EditUser extends Notification
{
    use Queueable;
    ...
    public function toMail()
    {
        ...
        $mailMessage->subject('Notificação de alteração de usuário');
        $mailMessage->greeting('Olá ' . FormatHelpers::first_word($this->collection->name) . ',');
        $mailMessage->line('Você está recebendo este e-mail porque foi realizado uma alteração de usuário na sua conta no sistema ' . config('app.name') . ', segue abaixo as alterações realizadas:');
        ...
        $mailMessage->line('<span class="notice">Se você desconhece está solicitação de alteração de usuário, procure o administrador do sistema.</span>');
