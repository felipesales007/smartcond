...
class BlockUser extends Notification
{
    ...
    public function toMail()
    {
        ...
        if ($this->blocked) {
            $mailMessage->subject('Notificação de bloqueio de usuário');
            $mailMessage->greeting('Olá ' . FormatHelpers::first_word($this->name) . ',');
            $mailMessage->line('Você está recebendo este e-mail porque foi realizado o <span class="text-warning">bloqueio do seu usuário' . $this->blocked . '</span>no sistema ' . config('app.name'));
            ...
            $mailMessage->subject('Notificação de desbloqueio de usuário');
            $mailMessage->greeting('Olá ' . FormatHelpers::first_word($this->name) . ',');
            $mailMessage->line('Você está recebendo este e-mail porque foi realizado o <span class="text-success">desbloqueio do seu usuário</span> no sistema ' . config('app.name'));
