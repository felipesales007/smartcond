...
class RecoverUser extends Notification
{
    use Queueable;
    ...
    public function toMail()
    {
        ...
        $mailMessage->subject('Notificação de recuperação de usuário');
        $mailMessage->greeting('Olá ' . FormatHelpers::first_word($this->name) . ',');
        $mailMessage->line('Você está recebendo este e-mail porque foi realizado a <span class="text-success">recuperação do seu usuário</span> no sistema ' . config('app.name'));
