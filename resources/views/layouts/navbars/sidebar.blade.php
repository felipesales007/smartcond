                                                ...
                                                @if (\App\Models\Menu\Menu::getMenu($item->menu_id)['menu_option_id'] == 1)
                                                    <li class="nav-item">
                                                        @if (!$item->button && $item->hidden == 0)
                                                            <!-- collapse item link -->
                                                            <a href="{{ $group_blocked || $route_blocked || $menu['blocked'] || $item['blocked'] ? 'javascript:void(0)' : route(\App\Models\Route\Route::getRoute($item->route_id)['route']) }}" class="nav-link fe-loading {{ $group_blocked ? 'notify-block-group' : '' }} {{ $route_blocked ? 'notify-block-route' : '' }} {{ $menu['blocked'] || $item['blocked'] ? 'fe-menu-block' : '' }} {{ Illuminate\Support\Facades\Request::is(\App\Models\Route\Group::getGroup($group_id)['name'] . '/' . $url) ? 'text-primary' : '' }}">
                                                        ...
                                                        @elseif (!$item->button && $item->hidden == 1)
                                                            ...
                                                            <a href="{{ $group_blocked || $route_blocked || $menu['blocked'] || $item['blocked'] ? 'javascript:void(0)' : $_SERVER['REQUEST_URI'] }}" class="nav-link fe-loading {{ $group_blocked ? 'notify-block-group' : '' }} {{ $route_blocked ? 'notify-block-route' : '' }} {{ $menu['blocked'] || $item['blocked'] ? 'fe-menu-block' : '' }} {{ Illuminate\Support\Facades\Request::is(\App\Models\Route\Group::getGroup($group_id)['name'] . '/' . $url) ? 'text-primary' : 'd-none' }}">
                                                                @if (Illuminate\Support\Facades\Request::is($group_name . '/' . $url))
                                                                    <i class="fas fa-circle fe-fs-6 ml--3 mr--3"></i>
                                                                @endif
                                                                {{ __(substr_replace($item->name, (strlen($item->name) > $str_limit ? '...' : ''), $str_limit)) }}
                        ...
                        @elseif ($menu->menu_option_id == 3)
                            ...
                            <li class="nav-item">
                                @if ($menu->hidden == 0)
                                    @if (!$button['button'])
                                        <!-- titulo link -->
                                        <a href="{{ $group_blocked || $route_blocked || $menu['blocked'] || $menu_item['blocked'] ? 'javascript:void(0)' : route(\App\Models\Route\Route::getRoute($route_id)['route']) }}" class="nav-link fe-loading {{ $group_blocked ? 'notify-block-group' : '' }} {{ $route_blocked ? 'notify-block-route' : '' }} {{ $menu['blocked'] || $menu_item['blocked'] ? 'fe-menu-block' : '' }} {{ $url ? 'active text-primary' : '' }}">
                                            <i class="{{ $menu->icon }} {{ \App\Models\Color::getColor($menu->color_id)['color'] }}"></i>
                                            <span class="nav-link-text">{{ __(substr_replace($menu->name, (strlen($menu->name) > $str_limit ? '...' : ''), $str_limit)) }}</span>
                                        </a>
                                ...
                                @else
                                    <!-- titulo link oculto -->
                                    <a href="{{ $group_blocked || $route_blocked || $menu['blocked'] || $menu_item['blocked'] ? 'javascript:void(0)' : url()->current() }}" class="nav-link fe-loading {{ $group_blocked ? 'notify-block-group' : '' }} {{ $route_blocked ? 'notify-block-route' : '' }} {{ $menu['blocked'] || $menu_item['blocked'] ? 'fe-menu-block' : '' }} {{ $url ? 'active text-primary' : 'd-none' }}">
                                        <i class="{{ $menu->icon }} {{ \App\Models\Color::getColor($menu->color_id)['color'] }}"></i>
                                        <span class="nav-link-text">{{ __(substr_replace($menu->name, (strlen($menu->name) > $str_limit ? '...' : ''), $str_limit)) }}</span>
                                    </a>
                                @endif
