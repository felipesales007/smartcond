...
function limpaFormCepCompanyNew() {
    let screen = '-new-company';
...
$(document).on('focusout', '#postal-code-new-company', function () {
    let screen = '-new-company';
...
cepCompanyNew = conteudo => {
    ...
    if (!('erro' in conteudo)) {
        ...
        returnStateIdCompanyNew(conteudo);
        ...
        limpaFormCepCompanyNew();
    }
};

// pesquisa o cep inserido
function viacepCompanyNew(valor) {
    ...
    if (cep !== '') {
        ...
        if (validacep.test(cep)) {
            ...
            script.src = 'https://viacep.com.br/ws/' + cep + '/json/?callback=cepCompanyNew';
...
function returnStateIdCompanyNew(conteudo) {
    let screen = '-new-company';
    let state_id;
