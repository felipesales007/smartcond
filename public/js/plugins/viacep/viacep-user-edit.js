...
function limpaFormCepUserEdit() {
    let screen = '-edit-user';
...
$(document).on('focusout', '#postal-code-edit-user', function () {
    let screen = '-edit-user';
...
cepUserEdit = conteudo => {
    ...
    if (!('erro' in conteudo)) {
        ...
        returnStateIdUserEdit(conteudo);
        ...
        limpaFormCepUserEdit();
    }
};

// pesquisa o cep inserido
function viacepUserEdit(valor) {
    ...
    if (cep !== '') {
        ...
        if (validacep.test(cep)) {
            ...
            script.src = 'https://viacep.com.br/ws/' + cep + '/json/?callback=cepUserEdit';
...
function returnStateIdUserEdit(conteudo) {
    let screen = '-edit-user';
    let state_id;
